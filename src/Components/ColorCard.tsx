...
import { fullColorHex, extractPixelData } from 'hadenajs';
...
const getPalette = async (photo: string) => {
  let pixels = await imagine(photo);
  let colors = await fetch('/api/palette', {
    method: 'POST',
    body: JSON.stringify({
      pixels: [pixels],
      k: 6
...
export const ColorCard: FC<Props> = ({ displayColor, image, size, isLoading, link, setStatus, error, units, blur, thumb }: Props) => {
  ...
  let errorCount = 0;
  const fetchPalette = (img: string): void => {
    setPalette([]);
    let now = Date.now();
    console.log('Running palette query at ', now);
    getPalette(img).then(res => {
      setPalette([displayColor, ...res.colors[0]]);
      ...
      if (errorCount < 3) {
        ...
        console.log('This palette query took ', Date.now() - now);
      }
    });
  };
  ...
  return (
    <div className={`relative rounded-lg ${isLoading || error ? 'animate-pulse' : ''}`} style={
      {
        backgroundSize: 'cover',
        background: showImage ? `transparent url('https://blurhash-decoder.function.cloud/?blurhash=${blur}') 0 0 / 100% 100%` : `rgb(${color.r}, ${color.g}, ${color.b})`,
        gridRow: `span ${size}`
      }
    }>
      <div className={`relative w-full h-full rounded-lg ${isLoading || error ? 'animate-pulse' : ''}`}
        style={
          {
            backgroundSize: 'cover',
